...
# =============================================================================
# 预扫描: 一次 API 调用获取文件列表缓存（避免每个子文件夹重复调 API）
# =============================================================================
echo -e "${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${YELLOW} [预扫描] 获取 ${ENV} 下所有 parquet 文件列表（仅调一次 API）${NC}"
echo -e "${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
...
if [ $? -ne 0 ]; then
    echo -e "${RED}❌ 文件列表扫描失败，终止批量处理${NC}"
    exit 1
fi
echo ""

# =============================================================================
# 逐个处理（每个子文件夹：下载→解压→转标注→清理，硬盘上同时只有一份数据）
# =============================================================================
for i in "${!ALL_SUBFOLDERS[@]}"; do
    ...
    # 调用完整 pipeline（使用缓存的文件列表，不再调 API 扫描）
    LOG_FILE="/tmp/openfly_pipeline_${ENV}_${SUBFOLDER}.log"
    USE_CACHE=1 bash scripts/run_pipeline.sh "$ENV" "$SUBFOLDER" 2>&1 | tee "$LOG_FILE"
