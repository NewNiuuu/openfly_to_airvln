...
# =============================================================================
# 预扫描: 尝试一次 API 调用缓存文件列表（失败则降级为逐个扫描）
# =============================================================================
echo -e "${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${YELLOW} [预扫描] 获取 ${ENV} 下 parquet 文件列表${NC}"
echo -e "${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""

python scripts/download_parquet.py --env "$ENV" --scan-only
SCAN_OK=$?

if [ $SCAN_OK -eq 0 ]; then
    echo -e "${GREEN}✅ 文件列表已缓存，后续下载无需调 API${NC}"
    USE_CACHE_FLAG="USE_CACHE=1"
else
    echo -e "${YELLOW}⚠️  预扫描失败（可能被限流），将逐个子文件夹单独扫描下载${NC}"
    USE_CACHE_FLAG=""
fi
echo ""

# =============================================================================
# 逐个处理（每个子文件夹完整走6阶段，硬盘上同时只有一份数据）
# =============================================================================
for i in "${!ALL_SUBFOLDERS[@]}"; do
    ...
    # 调用完整 pipeline（如有缓存则从缓存下载，否则单独调 API）
    LOG_FILE="/tmp/openfly_pipeline_${ENV}_${SUBFOLDER}.log"
    eval "${USE_CACHE_FLAG}" bash scripts/run_pipeline.sh "$ENV" "$SUBFOLDER" 2>&1 | tee "$LOG_FILE"
